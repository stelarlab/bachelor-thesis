...
class StripModel(nn.Module):
    def __init__(self, n_strip_feats=5, n_global_feats=3, d_model=64, n_heads=4, n_layers=2, dropout=0.1):
        super().__init__()
        self.strip_encoder = nn.Sequential(   # MLP pro Strip
        ...
        self.transformer = nn.TransformerEncoder(layer, num_layers=n_layers)  # = Message Passing
        self.attn_pool = nn.Linear(d_model, 1)
        self.global_proj = nn.Linear(n_global_feats, d_model)
        self.head = nn.Sequential(           # 3*d_model (mean+max+attn) + d_model (globals)
