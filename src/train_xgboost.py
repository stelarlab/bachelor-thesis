"""XGBoost v8b: 24 Features + optimierte Hyperparameter (12000 Runden, huber=0.3, depth=9)."""
...
def main():
    print("[XGB] Daten laden ...", flush=True)
    ev = load_events(DATA)
    ...
    if detector_shift != 0.0:
        ...
        print("[XGB] kein Detector-Shift gesetzt", flush=True)
    ...
    # v2 Hyperparameter vs. v1:
    #   huber_slope  1.0 -> 0.5  (robuster gegen Ausreisser)
    #   max_depth    8   -> 7    (weniger Overfitting)
    #   eta          0.05-> 0.03 (kleinere Schritte)
    #   num_rounds   1500-> 3000 (early_stopping=80)
    #   min_child_w  5.0 -> 8.0  (bessere Generalisierung)
    #   reg_lambda   1.0 -> 2.0  (staerkere L2)
    # v8 vs v4: num_boost_round 3000->8000, early_stopping 80->200
    #            huber_slope 0.5->0.3 (schaerfer auf Kern), max_depth 7->9
    params = dict(
    ...
    print("[XGB] gespeichert: outputs/xgb_predictions.npz", flush=True)
